...
class ESCursor:
	...
	def __init__(self, es_conn, index, doc_type="_doc", body=default_query_body, 
		...
		# If it's larger than the actual total number, use the actual total number instead.
		...
		self.current_batch_size = 0 # the actual batch size. May be smaller than "batch_size".
	...
	def _fetch_next_batch(self):
		...
		res_data = self.es_conn.search(index=self.index, doc_type=self.doc_type, body=body_batch)
